"""Single bounded WP3 scene probe; no queue, retry or training entry."""
...
def main():
    ...
    selected = next(line.split(",") for line in gpu.splitlines() if line.split(",")[0].strip()=="1")
    if int(selected[2]) < 24576:
        raise RuntimeError("insufficient free memory on GPU1")
    ...
    command=[str(parent/"venv/bin/python"),"-m","adaptation.pour17_v1.wp3_scene_probe",
        "--bundle-root",str(bundle),"--robot-urdf",str(parent/"assets/vega_1p_sharpa_fix.urdf"),
        "--reference",str(reference),"--output-dir",str(output),"--control-steps",str(args.control_steps),"--device","cuda:0","--headless","--enable_cameras"]
    record=dict(command=command,started=time.strftime("%Y-%m-%dT%H:%M:%S%z"),gpu=gpu,
        other_processes_untouched=processes,status="RUNNING",source_sha256={p.name:sha256(p) for p in (Path(__file__),Path(__file__).with_name("wp3_scene_probe.py"))})
    record_path.write_text(json.dumps(record,indent=2)+"\n")
    start=time.perf_counter()
    with (root/(args.name+"_console.log")).open("w") as log:
        result=subprocess.run(command,stdout=log,stderr=subprocess.STDOUT,
            env=dict(os.environ,CUDA_VISIBLE_DEVICES="1",PYTHONPATH=str(root),TMPDIR=str(parent/"tmp"),XDG_CACHE_HOME=str(parent/"cache")))
