...
class SharpaCorrectionEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        ground_usd = os.environ.get("RL_LOCAL_GROUND_USD")
        if ground_usd:
            if not os.path.isfile(ground_usd):
                raise FileNotFoundError(f"local ground USD not found: {ground_usd}")
            ground_cfg = GroundPlaneCfg(usd_path=ground_usd)
        else:
