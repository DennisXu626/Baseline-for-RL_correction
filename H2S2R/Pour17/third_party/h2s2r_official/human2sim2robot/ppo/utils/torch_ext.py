...
def safe_load(filename: Path) -> Any:
    ...
    # Checkpoints produced by the official trainer contain NumPy scalar
    # metadata.  PyTorch 2.6 changed ``torch.load`` to ``weights_only=True`` by
    # default, which rejects that official checkpoint format.  These files are
    # generated locally by this trainer, so retain the pre-2.6 loading behavior
    # explicitly rather than depending on the installed PyTorch default.
    load_fn = partial(
        torch.load,
