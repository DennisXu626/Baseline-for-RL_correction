...
class Pour17H2S2REnvCfg(DexmateCorrectionEnvCfg):
    ...
    clip_obs = 5.0
    decimation = 12
    # build_cfg replaces this for training with the released H2S2R
    # trajectory-length rule.  External evaluation alone keeps 903 steps.
    ...
    reference_length_frames = 0
    training_horizon_steps = 0
    evaluation_horizon_steps = 903
    provenance_manifest = ""
    right_synergy_npz = ""
    left_synergy_npz = ""
    ...
    cup_friction = 0.5
    body_friction = 0.2
    # Run10/11 remove the adaptation-only object pose/velocity rewrite.  The
    # near-grasp object is dynamic from the first control step.
...
def build_cfg(
    *,
    ...
    with np.load(cfg.perception_npz, allow_pickle=False) as archive:
        row_count = int(archive["frame_ids"].shape[0])
    if reference_start_index >= row_count - 1:
        raise ValueError(
            "reference_start_index must leave at least two source frames; "
            f"got {reference_start_index} for {row_count} frames"
        )
    cfg.reference_length_frames = row_count - reference_start_index
    # Released H2S2R: maxEpisodeLength = int(reference_length / min_speed),
    # where the minimum sampled speed factor is 0.5.
    cfg.training_horizon_steps = int(cfg.reference_length_frames / 0.5)
    ...
    horizon_steps = (
        cfg.evaluation_horizon_steps
        if cfg.external_evaluator_controls_termination
        else cfg.training_horizon_steps
    )
    cfg.episode_length_s = float(horizon_steps * control_dt)
    if int(round(cfg.episode_length_s / control_dt)) != horizon_steps:
        raise ValueError("episode horizon does not map exactly to control steps")
    return cfg
