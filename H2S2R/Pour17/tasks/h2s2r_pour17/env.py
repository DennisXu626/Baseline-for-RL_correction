...
class Pour17H2S2REnv(DirectRLEnv):
    ...
    def _verify_frozen_world(self, canonical: dict, manifest: dict) -> None:
        ...
        if self.cfg.external_evaluator_controls_termination:
            ...
            if self.cfg.warmup_clamp_steps != 0:
                raise ValueError("Run10/11 phase-one training requires object hold 0")
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self.controller.right.set_targets(
            self._actions[:, :SIDE_ACTION_DIM], palm_position_offset=offsets[:, 0]
        )
        left_q = self.controller.left.step(
            self._actions[:, SIDE_ACTION_DIM:], palm_position_offset=offsets[:, 1]
        )
        self._target_q.copy_(self.hand.data.joint_pos)
        self._target_q[:, self.side_joint_ids[ControlledSide.LEFT]] = left_q

    def _apply_action(self) -> None:
        right_q = self.controller.right.advance()
