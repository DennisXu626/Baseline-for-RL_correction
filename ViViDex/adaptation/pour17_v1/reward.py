...
def object_mimic_bimanual(
    *,
    ...
    hand_object_contacts: torch.Tensor,
    ...
    Hand points are (N,2,6,3): palm then five tips. The two side rewards are
    ...
    tip_error = torch.linalg.vector_norm(
        actual_hand_points[..., 1:, :] - target_hand_points[..., 1:, :], dim=-1
    ).mean(dim=-1)
    ...
    any_contact = hand_object_contacts
    ...
    hand_reward = 4.0 * torch.exp(-config.hand_error_scale * tip_error)
